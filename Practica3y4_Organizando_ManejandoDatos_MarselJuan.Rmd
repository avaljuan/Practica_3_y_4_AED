...
gather(pew_religion, Income, freq,-religion,factor_key = TRUE)
...
```{r}
#Falta hacer bien
df_tidy <- raw %>% pivot_longer(names_to = "week", values_to = "rank", cols = starts_with("wk"),names_prefix = "wk")


#df_tidy <- df_tidy %>%
  mutate(semanas_top = as.Date(date.entered) + weeks(as.numeric(week) - 1))
...
rm(list=ls())
load('data/tb.Rdata')
raw_clean <- raw%>%gather(key = "sex",value="cases",-country,-year) %>% separate(col="sex",into = c("sex","age"), sep=1)%>%drop_na() %>% mutate(age=as.factor(age))

View(raw_clean)
```
```{r}
levels(clean$age)
levels(raw_clean$age)
```
...
rm(list=ls())
load("data/weather.RData")
raw_clean <- raw %>% pivot_longer(names_to= "dia", values_to="valor", cols=starts_with("d"), names_prefix = "d") %>% pivot_wider(names_from = element,values_from = valor) %>% drop_na() %>% mutate(date = as.Date(paste0(year,"/",month,"/",dia),format="%Y/%m/%d")) %>% select(id,date,tmin,tmax)
...
```{r }
planes.f <- planes %>% 
  filter(!str_detect(str_to_lower(manufacturer),pattern =".*douglas.*")) %>% 
...
```{r }
flights %>% 
  mutate(Full_Date = as.Date(paste(year,month,day,sep="/")), format="%Y/%m/%d" )%>% select(Full_Date,1:(ncol(flights)-1))
